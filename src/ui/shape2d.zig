...
pub const ShapeCommand = struct {
    ...
    pub fn pushClip(x: f32, y: f32, w: f32, h: f32) !void {
        try ui.state.shape_cmds.append(.{ .command = .{ .clip = .{
            .x = x,
            .y = y,
            .w = w,
            .h = h,
    ...
    pub fn pushRect(x: f32, y: f32, w: f32, h: f32, color: u24) !void {
        try ui.state.shape_cmds.append(.{ .command = .{ .rectangle = .{
            .dim = .{
                .x = x,
                .y = y,
                .w = w,
    ...
    pub fn pushText(x: f32, y: f32, color: u24, string: []const u8) !void {
        try ui.state.shape_cmds.append(.{
            .command = .{
                .text = .{
                    .x = x,
                    .y = y,
    ...
    pub fn lastCommandInList(ui_cmd: *ShapeCommand) *ShapeCommand {
        var cmd = ui_cmd;
        while (cmd.next != null) cmd = cmd.next.?;
        return cmd;
    }

    pub fn listCount(shape_command: *ShapeCommand) u32 {
        var shape_cmd: ?*ShapeCommand = shape_command;
        var count: u32 = 0;
        while (shape_cmd) |cmd| {
            count += 1;
            shape_cmd = cmd.next;
