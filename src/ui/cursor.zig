...
pub fn render(rect: Rect, renderer_text: *Text, window: Window, cursor: Cursor, string: []const u8, color: vectors.vec3) void {
    var cursor_x = @floatToInt(i32, window.x);
    ...
    while (it.next()) |text_segment| : (i += 1) {
        ...
        if (text_segment.is_ascii) {
            ...
            cursor_x += @intCast(i32, character.Advance >> 6);
        } else {
    ...
    var cursor_y = (@intCast(i32, cursor.row - 1) * initial_cursor_h) + @floatToInt(i32, window.y) + initial_cursor_h;
    var cursor_h = @divFloor(initial_cursor_h, 2) + 5;
